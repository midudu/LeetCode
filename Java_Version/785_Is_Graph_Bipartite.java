...
class Solution {
    ...
    public static void main(String[] args) {
        ...
        boolean result = new Solution().isBipartite(graph);
    ...
    public boolean isBipartite(int[][] graph) {
        ...
        //DFS
        // 0(not meet), 1(black), -1(white)
        int[] visited = new int[graph.length];

        for(int i = 0; i < graph.length; i++){

            if( visited[i] == 0 && !isBipartiteHelper(visited, graph, 1, i)){
                return false;
            }
        }
        return true;
    ...
    private boolean isBipartiteHelper(int[] visited, int[][] graph, int color, int cur) {
        ...
        for(int nextNode: graph[cur]){
            if(!isBipartiteHelper(visited, graph, -color, nextNode))
                return false;
        }
