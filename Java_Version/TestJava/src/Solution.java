...
class Solution {
    ...
    public int wiggleMaxLength(int[] nums) {
        ...
        int signChangedCount = 0;
        int lastSign = 0;

        for (int i = 1; i < nums.length; i++) {

            int currentSign = signOfTwoNumbers(nums[i - 1], nums[i]);
            if (lastSign == 0 && currentSign != 0) {
                signChangedCount++;
            } else if (lastSign != 0 && currentSign + lastSign == 0) {
                signChangedCount++;
            }
        ...
        return signChangedCount + 1;
    }

    private int signOfTwoNumbers(int num1, int num2) {
